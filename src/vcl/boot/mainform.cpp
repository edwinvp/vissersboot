...
//---------------------------------------------------------------------------
float __fastcall TMainFrm::Pwm2MotorFact2(unsigned char hi, unsigned char lo)
{
	int combined = ((hi&0x3) << 8) + lo;
    return (combined - 0x180) / 128.0;
}
//---------------------------------------------------------------------------
...
void __fastcall TMainFrm::Timer1Timer(TObject *Sender)
{
	...
	if (CbRemoteCtrlOn->Checked) {
		k1_pulse_duration=SbMotorV->Position;
		k2_pulse_duration=SbMotorH->Position;
		k3_pulse_duration=4000 - (SbPos->Position - 2000);
		k4_pulse_duration=SbManAuto->Position;
	} else {
	...
	avr_motor_l = Pwm2MotorFact2(TC4H,OCR4D); // left motor
	avr_motor_r = Pwm2MotorFact(OCR3A);
	...
	VarForm->AddLine(L"---");
	VarForm->AddLine(L"k1 pd = " + FloatToStr(k1_pulse_duration));
	VarForm->AddLine(L"k2 pd = " + FloatToStr(k2_pulse_duration));
	VarForm->AddLine(L"k3 pd = " + FloatToStr(k3_pulse_duration));
	VarForm->AddLine(L"k4 pd = " + FloatToStr(k4_pulse_duration));
